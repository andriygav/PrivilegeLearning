...
\end{aligned}
\end{gather}
где~$g^k$ обозначает предсказаную вероятность~$$
\end{enumlist}
\begin{theorem}
...
\end{theorem}
\begin{proof}
	Во-первых покажем, что для произвольного $\mathbf{s} \in \mathcal{S}_K$ выполняется $p\bigr(\mathbf{s}|\mathbf{x}, \mathbf{g}\bigr) \geq 0$. Заметим, что для всех~$k$ выполняется, что~$\log g_k\bigr(\mathbf{x}\bigr) < 0,$ тогда
\begin{gather}
\begin{aligned}
...
тогда с учетом того, что~$g_k\bigr(\mathbf{x}\bigr) >0$ и~$C>0$ получаем, что $p\bigr(\mathbf{s}|\mathbf{x}, \mathbf{g}\bigr) \geq 0$.
	Во-вторых покажем, что интеграл по всему пространству объектов~$\mathcal{S}_K$ является конечным:
	\begin{gather}
...
\begin{enumlist}
	\item рассматривается функция учителя~$\mathbf{f}\in\mathcal{F}_{\text{rg}}^{*}$:
	\begin{gather}
	\label{eq:F:set:priv}
	\begin{aligned}
	...
	\item рассматривается функция ученика~$\mathbf{g}\in\mathcal{G}_{\text{rg}}$:
\begin{gather}
\label{eq:G:set:rg}
...
где~$\mathbf{z}$~--- это дифференцируемая параметрическая функция;
	\item истинные метки имеют нормальное распределение распределение
	\begin{gather}
